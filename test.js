...
// GLOBAL DEFINITIONS (simulate browser environment)
//
////////////////////////////////////////////////////
...
var counter = 10
var currId = 1;
var detailRootGuid = "b49d39c9-b903-cccd-7d32-b84beb1b76dc";
...
var uccelloClt = new UccelloClt( {
    callback: function () {
        ...
        login(_user, _pwd, function (res) {
            logger.info("Login: " + res);
            switch (_test_num) {
                case 1:
                    testMoveCursor(uccelloClt);
                    break;
                case 2:
                case 3:
                    testLoadNewRoots(uccelloClt);
                    break;
                default:
                    throw new Error("### Unknown test number: " + _test_num);
            };
        });
    },
    newTabCallback: function () {
        console.log("uccelloClt.newTabCallback");
    },
    commClient: commClient
});

function testMoveCursor(uccelloClt) {
    var visualContext;
    var detailRoot;
    uccelloClt.createContext('server', [testFormGuid], function (result) {
        logger.info("Context was loaded !");
        visualContext = uccelloClt.getSysCM().getObj(result.vc);
        uccelloClt.setContext({ vc: result.vc, side: result.side, formGuids: result.roots, urlFormGuids: 'all' }, function (arg) {
            logger.info("Context was set !");
            if (!result.roots[0])
                throw new Error("Invalid result.roots[0]: " + JSON.stringify(result.roots[0]));
            internalTest();
            function internalTest() {
                var mainForm = visualContext.getContentDB().getObj(result.roots[0]);
                if (!mainForm) {
                    //throw new Error("Missing object \"mainForm\": " + JSON.stringify(result.roots[0]));
                    var msg = "Missing object \"mainForm\": " + JSON.stringify(result.roots[0]);
                    logger.error(msg);
                    logger.info(msg);
                    setTimeout(internalTest, 10000);
                } else {
                    var datasetCompany = visualContext.getContentDB().getObjByRoot(mainForm, "061b1347-18a2-a829-fcce-ba59058b4bdf");
                    var datasetAddress = visualContext.getContentDB().getObjByRoot(mainForm, "92720532-a58f-68bb-6e5a-02b28239e312");
                    visualContext.loadNewRoots([datasetCompany.getSerialized("ObjType")], { rtype: "data" }, function (res) {
                        visualContext.loadNewRoots([datasetAddress.getSerialized("ObjType")], { rtype: "data", expr: 1 }, function (res) {
                            logger.info("New root was loaded !");
                            var curr_item = 0;
                            var dbVer;
                            function moveCursor(res) {
                                var cursor = datasetAddress.cursor();
                                if (typeof cursor == "number") {
                                    var colAddress = datasetAddress.root().getCol("DataElements");
                                    totOp++;
                                    
                                    var newVer = (res && res.delta) ? res.delta.dbVersion : ((res && res.data) ? res.data.dbVersion : null);

                                    if (dbVer && res && ((newVer - dbVer) != 1)) {
                                        console.log("### WARNING: prev version: " + dbVer + ", next: " + newVer + ".");
                                    };
                                    dbVer = visualContext.getContentDB().getVersion("valid");
                                    curr_item++;
                                    if (curr_item >= colAddress.count())
                                        curr_item = 0;
                                    datasetAddress.cursor(colAddress.get(curr_item).id());
                                    visualContext.getContentDB().getController().genDeltas(visualContext.getContentDB().getGuid(), moveCursor);
                                    //visualContext.getContentDB().getController().genDeltas(visualContext.getContentDB().getGuid(), true, moveCursor);
            ...
            //logger.info("Render root.");
        });
    });
};

function testLoadNewRoots(uccelloClt) {
    var visualContext;
    var detailRoot;
    ...
    uccelloClt.createContext('server', [testFormGuid], function (result) {
        ...
        visualContext = uccelloClt.getSysCM().getObj(result.vc);
        uccelloClt.setContext({ vc: result.vc, side: result.side, formGuids: result.roots, urlFormGuids: 'all' }, function (arg) {
            visualContext.loadNewRoots([detailRootGuid], { rtype: "data", expr: currId++ }, function loadDetail(res) {
                if (isFirstTime) {
                    logger.info("New root loaded !");
                    isFirstTime = false;
                };
                totOp++;
                if ((opt_load === "fast") && (res.guids[0]))
                    detailRoot = visualContext.getContentDB().getObj(res.guids[0]);
                if (currId > counter)
                    currId = 1;
                if (opt_load === "fast")
                    visualContext.loadNewRoots([detailRoot.getGuid()], { rtype: "data", expr: currId++ }, loadDetail);
                else
                    visualContext.loadNewRoots([detailRootGuid], { rtype: "data", expr: currId++ }, loadDetail);
            });
        }, function () {
            //logger.info("Render root.");
        });
...
function login (name, pass, done) {
    var session = { guid: uccelloClt.getSessionGuid(), deviceName: 'MyComputer', deviceType: 'C', deviceColor: '#6ca9f0' };
    uccelloClt.getClient().authenticate({ user: name, pass: pass, session: session }, function (result) {
        if (result.user) {
            uccelloClt.subscribeUser(function (result2) {
                if (!result2) {
                    logger.error("subscribeUser failed.");
                } else {
...
setTimeout(function showDbs() {
    var end = new Date();
    console.log("### [" + getTestNameAndSetOptions(_test_num) + "] Total ops: " + totOp + ", time: " + (end - start) + " ms, rate: " 
        + (totOp * 1000 / (end - start)).toFixed(2) + " op/sec.");
    start = end;
    totOp = 0;
    setTimeout(showDbs, _timeout);
}, _timeout);
